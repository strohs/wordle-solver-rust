...
pub struct Solver {
    /// remaining words, Vec of (word, goodness, index of word in cache)
    remaining: Cow<'static, Vec<(&'static str, f64, usize)>>,
    entropy: Vec<f64>,
    options: Options,
    /// index into Cache of last guessed word
...
impl Solver {
    ...
    /// filters the remaining word list by keeping words that pass the given `cmp` function
